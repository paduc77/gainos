...
#define CANSM_VALIDATE(expression, serviceId, errorId, __rv)	\
	if (!(expression)) {									\
...
#define CANSM_VALIDATE_INIT(serviceID, __rv)					\
		CANSM_VALIDATE((CanSM_Internal.InitStatus == CANSM_STATUS_INIT), serviceID, CANSM_E_UNINIT, __rv)
...
#define CANSM_VALIDATE_NETWORK(net, serviceID, __rv)					\
		CANSM_VALIDATE( (net < CANSM_NETWORK_COUNT), serviceID, CANSM_E_INVALID_NETWORK_HANDLE, __rv)

#define CANSM_VALIDATE_MODE(mode, serviceID, __rv)					\
		CANSM_VALIDATE( (mode <= COMM_FULL_COMMUNICATION) && (mode != COMM_SILENT_COMMUNICATION), serviceID, CANSM_E_INVALID_NETWORK_MODE, __rv)
